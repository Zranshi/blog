...
冒泡排序是每次比较下标相邻的元素, 然后逐步向后遍历. 每次可以将最大的那个数放置到未排序序列的末尾, 因此最多需要循环n次. 但是在每次冒泡过程中, 也将其他元素进行了排序. 所以可以通过一个变量来监测当前循环中有无变化. 如果没有变化, 就不用继续进入下一个循环了.
...
希尔排序是直接插入排序的进阶版, 它由多个(一般为log[len(arr)])直接插入排序构成. 在每次直接插入排序中, 原本只是比较相邻的两个数, 但在希尔排序中, 每次会比较步长个间距的两个数.
...
以下是维护堆的代码. 我们设置这样的一个函数 heap_fit(target_idx, size) 其中 target_idx 表示目前需要调整的元素下标, size表示堆的最大下标.
