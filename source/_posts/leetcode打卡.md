...
* 节选自CSDN用户[九茶](https://blog.csdn.net/Bone_ACE)
* 分析链接：[https://blog.csdn.net/Bone_ACE/article/details/46239187](https://blog.csdn.net/Bone_ACE/article/details/46239187)
...
* 我们观察可知，这道题如果只用一重循环，是可以转化为leetcode第一题[两数相加](https://leetcode-cn.com/problems/two-sum/)，因此我们只用遍历数组一次，然后采用双指针首尾遍历一次就可以找到结果了。
* 但是需要注意的是，这道题要返回的是所有满足条件的不重复三元组，所以先用set保存，可以自动去重，最后再将set变为list返回。(需要注意的是，set中不能存list，所以用tuple保存)
